...
use documentdb_gateway::{get_service_context, populate_ssl_certificates, QueryCatalog, AUTHENTICATION_MAX_CONNECTIONS};
...
async fn run(config: DocumentDBSetupConfiguration) {
    ...
    let authentication_pool = ConnectionPool::new_with_user(
        ...
        &postgres_system_user,
        None,
        format!("{}-TestAuth", config.application_name()),
